...
public class CandidateDaoImpl extends Dao implements CandidateDao {
	...
	public CandidateDaoImpl() throws Exception {
		...
		String sqlChangeStatus = "UPDATE candidates SET status = 1 id = ?";
		stmtChangeStatus = con.prepareStatement(sqlChangeStatus);
	}
	...
	public int changeStatus(int candId) throws Exception {
		stmtChangeStatus.setInt(1, candId);
		int count = stmtIncrVote.executeUpdate();
